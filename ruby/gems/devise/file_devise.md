...
devise gemの`lib/devise/devise.rb`を読解する。
...
```rb
module Devise # Devise名前空間は、このgemの主幹になる
  module Controllers
  module Hooks
  module Mailers
  module Strategies
...
```rb
# これはこのファイル内の全ての文字列をimmutable(つまり.freezeしたのと同じ)にするマジックコメント。
# immutableにすることでパフォーマンスが向上するとか
...
module Devise
  ...
  # nodocというのは、Rubyのドキュメンテーション生成の対象からこのメソッドを除外するための記述
